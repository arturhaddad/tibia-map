(function () {
	function initializeMap(id = undefined) {
		function TibiaMap() {
			this.map = null;
		...
		let mapElement = document.getElementsByClassName("interactive-map")[0];
		...
		// Features: fullscreen, selectable, exiva
		...
		const setMapPosition = function (coords) {
			const coordinates =
				coords.x + "," + coords.y + "," + coords.floor + ":" + coords.zoom;
			mapData.coordinates = coordinates;
		};
		TibiaMap.prototype.setMapPosition = setMapPosition;

		const convertStringToPosition = function (string) {
		...
		const getMapPosition = function () {
			if (mapData.coordinates) {
				return convertStringToPosition(mapData.coordinates);
			}

			return defaultPosition;
		};

		TibiaMap.prototype.getMapPosition = getMapPosition;
		...
		TibiaMap.prototype.init = function () {
			...
			const map = (_this.map = L.map(mapElement.id, {
			...
			map.on("click", function (event) {
				if (isFeatureEnabled("selectable")) {
					...
					setMapPosition({ x: coordX, y: coordY, floor: coordZ, zoom: zoom });
					const formatedCoords = coordX + "," + coordY + "," + coordZ;
					window.postMessage(
						{ coords: formatedCoords, mapId: mapData.customId },
	...
	const handleMapRefetch = (event) => {
		if (event.data.refetchMap) {
			initializeMap(event.data.mapId);
		}
	};

	window.addEventListener("message", handleMapRefetch);
})();
