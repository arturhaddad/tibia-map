L.LevelButtons = L.Control.extend({
	...
	onAdd: function (map) {
		this._map = map;
		const plugin_container = L.DomUtil.create(
			"div",
			"leaflet-control-level-buttons-panel leaflet-bar"
		);

		const up_button = L.DomUtil.create(
			"a",
		...
		const floor_button = L.DomUtil.create(
			"span",
		...
		const down_button = L.DomUtil.create(
			"a",
			"leaflet-control-level-buttons-a",
			plugin_container
		);
	...
	_onUpButton: function (event) {
		...
		if (upper_floor_index >= 0) {
			...
			this._updateMapPosition(upper_floor_index);
		}
	...
	_onDownButton: function (event) {
		...
		if (lower_floor_index <= 15) {
			...
			this._updateMapPosition(lower_floor_index);
		}
	...
	_updateMapPosition: function (floor) {
		const coordinates = this._tibia_map_obj.getMapPosition();
		coordinates.floor = floor;
		this._tibia_map_obj.setMapPosition(coordinates);
	},
});
